...
class Auth {
  ...
  async loginUser(req, res, next) {
    ...
    if (admin) {
      ...
      const matchPassword = await bcrypt.compare(
        req.body.password,
        departement.password
      );
      if (!matchPassword)
        return res.status(404).json({ msg: "Password salah !" });
      req.session.userId = departement.id_dpm;
      const dataDepartement = {
        userId: departement.id_dpm,
        nama: departement.nama,
        noInduk: departement.noInduk,
        role: departement.role,
        kode_departement: departement.kode_departement
      };
      return res
        .status(200)
        .json({ data: dataDepartement , msg: "login sukses" });
    }
