...
const UpdateDepartement = ({ isUpdate, datas }) => {
  ...
  const getMahasiswaById = async () => {
    try {
      const response = await axios.get(
        ` http://localhost:5000/api/v1/departemen/${datas.id_dpm}`
      );
  ...
  const updateAdmin = async () => {
    ...
    try {
      await axios.put(` http://localhost:5000/api/v1/departemen/${datas.id_dpm}`, formData);
      reloadPage();
    } catch (error) {
      if (error.response) {
        setMsg(error.response.data.msg);
    ...
    <div>
      ...
      <Modal show={show} onHide={handleClose} size="lg" className="modal-css">
        <Modal.Header closeButton>
          <Modal.Title className="label ">* Update Data Departement</Modal.Title>
        </Modal.Header>
        <Modal.Body>
